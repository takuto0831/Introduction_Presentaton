...
# コンパイル
model1 <- stan_model("~/Desktop/Intro_pres/Bayes_Introduction/stan/glm_bayes_model.stan",
                     auto_write = FALSE)
# サンプリング
fit1 <- rstan::sampling(object = model1,
                        ...
                        chains = 3)
# savefile
saveRDS(fit1,"~/Desktop/Intro_pres/Bayes_Introduction/fit/glm_bayes.rds")
```
...
```{r}
fit1 <- readRDS("~/Desktop/Intro_pres/Bayes_Introduction/fit/glm_bayes.rds")
fit1
```
...
fit2 <- rstan::sampling(object = model2,
                        ...
                        chains = 3)
# save file
saveRDS(fit2,"~/Desktop/Intro_pres/Bayes_Introduction/fit/hierarchical_bayes.rds")
```

## サンプリング結果の確認

```{r}
fit2 <- readRDS("~/Desktop/Intro_pres/Bayes_Introduction/fit/hierarchical_bayes.rds")
fit2
```
...
- 給食タイプ$1$に変化させた5県における県ごとの差$\gamma_{3,j}$を追加する.
- 階層事前分布には half-t 分布を使用する
...
d_list <- list(N = 2,
               ...
               Age = t(matrix(d$Age, length(unique(d$pref)), 2)))
# コンパイル
model3 <- stan_model("stan/hierarchical_bayes_model2.stan",auto_write = FALSE)
# サンプリング
fit3 <- rstan::sampling(object = model3,
                        data = d_list,
                        verbose = TRUE, 
                        iter = 5000, 
                        chains = 3)
# save file
saveRDS(fit3,"~/Desktop/Intro_pres/Bayes_Introduction/fit/hierarchical_bayes2.rds")
```
...
```{r}
fit3 <- readRDS("~/Desktop/Intro_pres/Bayes_Introduction/fit/hierarchical_bayes2.rds")
fit3
```
