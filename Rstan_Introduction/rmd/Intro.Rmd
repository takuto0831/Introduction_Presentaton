...
```{r eval=FALSE}
model1 <- rstan::stan_model('stan/simple_model.stan') # モデルのコンパイル
...
```{r echo=FALSE}
model1 <- readRDS("../model/simple_model.rds") # モデルの読み込み
...
- Rhatとは, 複数のchainの結果から, MCMCサンプルの分散を比較して検出され,  MCMCの収束を評価する指標の一つである. (詳しくは参考文献の[2]を参照)
...
p1 <- ggs_histogram(fit1) # 事後分布のヒストグラム
p2 <- ggs_autocorrelation(fit1) # 事後分布の自己相関
p3 <- ggs_geweke(fit1) # Gewekeの診断方法
p4 <- ggs_Rhat(fit1) + xlab("R_hat") # 各パラメータのRhatのばらつき
```
